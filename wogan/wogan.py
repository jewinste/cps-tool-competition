...
from wogan.sut import SUT
from wogan.models import WGAN
...
class WOGAN:
    ...
    while self.executor.time_budget.can_run_a_test():
      ...
      while not found:
        ...
        if 1 - min(heap)[0] >= target_fitness:
          ...
          self.test_bins[self.get_bin(self.test_outputs[self.tests_generated, 0])].append(self.tests_generated)
      ...
      if (self.tests_generated - self.random_init) % self.train_delay == 0:
        ...
        for epoch in range(self.model.train_settings_init["epochs"]):
          ...
          for i in range(self.train_delay):
            if self.get_bin(self.test_outputs[self.tests_generated - i - 1]) >= self.bin_sample(1, self.S, self.R(self.get_elapsed_execution_time()))[0]:
  ...
  def generate_initial_random_tests(self):
    ...
    if self.executor.time_budget.time_budget is None:
      condition = lambda: self.executor.get_remaining_time()["execution-budget"] > self.executor.time_budget.execution_budget - self.init_budget
    else:
      condition = lambda: self.executor.get_remaining_time()["time-budget"] > self.executor.time_budget.time_budget - self.init_budget
